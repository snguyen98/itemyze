...
def upload_splitwise(_, id=None):
    ...
    if not allocations.exists():
        return Response(
            {"error": "Allocations not found."}, status=status.HTTP_404_NOT_FOUND
        )
    ...
    for index, val in enumerate(allocations):
        ...
        payload[f"users__{index}__paid_share"] = (
            payload["cost"] if val.splitwise_user == expense.splitwise_paid_by else 0
        )
    ...
    if not res["errors"]:
        ...
        return Response(
            {"message": f"Expense data sent to splitwise: {splitwise_id}"},
        ...
        return Response(
            {"error": res["errors"]}, status=status.HTTP_500_INTERNAL_SERVER_ERROR
        )
