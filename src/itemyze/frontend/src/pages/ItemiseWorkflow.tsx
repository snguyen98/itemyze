...
const ItemiseWorkflow = () => {
    ...
    const retrieveExpense = () => {
        if (expenseId !== undefined && expenseId > 0) {
            getExpense(expenseId)
                .then(res => {
                    setExpense(res);
                    getGroupMembers(res.splitwiseGroup)
                        .then(res => {
                            setUsers(res);
                        });
                });
    ...
    const validateAllocations = () => {
        const totalSum: number = Object.values(allocations).reduce((sum, allocation) => sum + Number(allocation.amount), 0);
        return totalSum > 0;
    }
    ...
    const overlayComponents = (stepNum: number): React.ReactNode => {
        switch (stepNum) {
            case 1: return (
                expense && expense.currencyUnit && items && 
                <ViewItems
                    items={items}
